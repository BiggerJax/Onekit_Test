...
+ (OKDB *)openOKDB:(NSString *)name
{
    return [OKDB openDB:name check:true];
}
+ (OKDB *)openDB:(NSString *)name check:(BOOL)check
{
    [self init];
    if (check && ![OKFso isExist:name]) {
    ...
    OKDB* db = [connPooling objectForKey:name];
    if (isNull(db)){
        db = [OKDB new];
...
{
    NSString *path;
    path = [OKFso getPathWithFileName:name];
...
+(void)openQuery:(NSString *)name sql:(NSString*)sql callback:(void(^)(NSDictionary *))callback params:(id)params,... {
    ...
    NSDictionary *dicArray = nil;
        ...
        if (result != SQLITE_OK) {
            NSLog(@"[OneKit-DB]:prepare error! %s",sqlite3_errmsg(db.Connection));
        }
        ...
        if (notNull(params)) {
            ...
            if (result != SQLITE_OK) {
                NSLog(@"[OneKit-DB]:firstArgument error! %s",sqlite3_errmsg(db.Connection));
