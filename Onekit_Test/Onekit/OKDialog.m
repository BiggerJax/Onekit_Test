...
@interface OKDialog()<UIAlertViewDelegate, UIActionSheetDelegate,UITableViewDataSource,UITableViewDelegate>
...
+ (id)shareInstance{
    if (isNull(m_Instance)) {
        m_Instance = [[OKDialog alloc] init];
    }
    return m_Instance;
}
       ...
       callback:(void (^)(void))callback
{
    ...
    OKDialog* m_Instance = [self shareInstance];
    m_Instance.alertViewCallback = ^(BOOL ok){
        if (notNull(callback)) {
            callback();
        }
    };
    [self showAlert:Nil message:message leftButton:@"我知道了" rightButton:Nil OKDialog:m_Instance];
}
...
+(void)showAlert:(NSString *)title message:(NSString *)message leftButton:(NSString *)leftButton rightButton:(NSString *)rightButton OKDialog:(OKDialog *)OKDialog{
...
+(void)alert:(NSString *)title callback:(void (^)(void))callback{
    OKDialog* m_Instance = [OKDialog shareInstance];
    m_Instance.alertViewCallback = ^(BOOL ok){
        if (notNull(callback)) {
            callback();
        }
    ...
    [self showAlert:Nil message:title leftButton:@"关闭" rightButton:Nil OKDialog:m_Instance];
}
...
+ (void)showWithText:(NSString *)text_
    ...
    [OKDialog showWithText:text_  bottomOffset:bottomOffset_ duration:DEFAULT_DISPLAY_DURATION];
}

+(void)showWithText:(NSString *)text_
       bottomOffset:(CGFloat)bottomOffset_
           duration:(CGFloat)duration_{
    OKDialog *toast = [[OKDialog alloc] initWithText:text_];
...
+ (void)confirm:(NSString *)message
    ...
    OKDialog* OKDialog;
    if ((OKDialog = [self shareInstance])) {
        OKDialog.alertViewCallback = okCallback;
        OKDialog.alertViewType = AlertViewTypeConfirm;
        [self showAlert:Nil message:message leftButton:noTitle rightButton:yesTitle OKDialog:OKDialog];
    }else{
        callback();
        return;
    }
...
cancelCallBack:(void(^)())cancelCallback
{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:title
                                                        message:nil
                                                       delegate:[OKDialog shareInstance]
    ...
    ((OKDialog *)[OKDialog shareInstance]).alertViewType = AlertViewTypeInput;
    ((OKDialog *)[OKDialog shareInstance]).inputCallback = okCallback;
...
+(void)input:(NSString *)title placeHolder:(NSString *)placeHolder type:(UIKeyboardType)keyboardType callback:(void (^)(BOOL, NSString *))callback{
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ((OKDialog*)[OKDialog shareInstance]).inputCallback = callback;
    }];
...
+(void)optionlist:(UIViewController*)viewController title:(NSString *)title options:(NSArray *)options cancel:(void (^)())cancelCallback{
    OKDialog* OKDialog = [self shareInstance];
    OKDialog.alertCallback = cancelCallback;
