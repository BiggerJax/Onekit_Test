 ...
 */
+(void)base642bytes:(NSString*)base64 callback:(void(^)(OKBytes *bytes))callback;
/*!
 *  image转bytes
 *  @param image  图像
 *  @param callback   返回bytes
 */
+(void)image2bytes:(UIImage *)image callback:(void(^)(OKBytes *bytes))callback;
/*!
 *  data转bytes
 *  @param data  二进制
 *  @param callback   返回bytes
 */
+(void)data2bytes:(NSData*)data callback:(void(^)(OKBytes *bytes))callback;
/*!
 *  bytes转image
 *  @param bytes  bytes数组
 *  @param callback   返回图像
 */
+(void)bytes2Image:(OKBytes*)bytes callback:(void(^)(UIImage *image))callback;
/*!
 *  bytes转base64
 *  @param bytes  bytes数组
 *  @param callback   返回大文件
 */
+(void)bytes2base64:(OKBytes*)bytes callback:(void(^)(NSString *string))callback;
/*!
 *  bytes转data
 *  @param bytes  bytes数组
 *  @param callback   返回二进制
 */
+(void)bytes2data:(OKBytes*)bytes callback:(void(^)(NSData *data))callback;
/*!
 *  base64转data
 *  @param base64  大文件
 *  @param callback   返回二进制
